...
Let $\textit{1TAPE}[T(n)]$ denote the family of languages accepted by deterministic one-way Turing machines with $T(n)$ work tape \cite{HM81}. A monotonically increasing function $f$ is not a constant and for every two values $x < y$ in the domain of $f$, then we obtain that $f(x) \leq f(y)$.

\begin{theorem}
\label{closed}
If $T(n)$ is a monotonically increasing function such that
\[\lim_{n\to\infty} \frac{T(n)}{n} = 0\]
then $\textit{1TAPE}[T(n)]$ is closed under deterministic two-way logarithmic space reduction.
\end{theorem}

\begin{proof}
Suppose that $\textit{1TAPE}[T'(n)]$ is not closed under deterministic two-way logarithmic space reduction for some monotonically increasing function $T'(n)$ when
\[\lim_{n\to\infty} \frac{T'(n)}{n} = 0.\]
...
\[\lim_{n\to\infty} \frac{T(n)}{n} = 0\]
in their paper \cite{HM81}. We can use this result when $T(n)$ is a monotonically increasing function as well \cite{HM81}. Hence, this is a consequence of Theorem \ref{closed}, because $1NL$ is also closed under deterministic two-way logarithmic space reduction.
\end{proof}
