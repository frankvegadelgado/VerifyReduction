...
\begin{theorem}
\label{partially}
$|1LNL|_{c} > |1L|_{c}$ where $|\ldots|_{c}$ denotes the cardinality. 
\end{theorem}

\begin{proof}
On the one hand, if we suppose that $|1NL|_{c} \neq |1L|_{c}$, then we obtain directly that $|1LNL|_{c} > |1L|_{c}$ since $1L \varsubsetneq 1NL$ and $1LNL$ contains $1NL$, because the prefix $y$ of the instance $x = yz$ could be the empty string. On the other hand, if we suppose that $|1NL|_{c} = |1L|_{c}$, then we can make a bijective relation between a language $L' \in 1L$ and another language $L'' \in 1NL$. Hence, we can create the languages $L''' \in 1LNL$ such that
...
As result, $L = NL$ if and only if $2L = 2NL$. We define the class $LNL$ as the languages
\[L' =\{yz: y \in L \textit{ and } z \in NL\}.\]
Therefore, we can deterministic logarithmic space reduce every language $L' \in LNL$ to a language $L_{1} \in 1LNL$ or to a language in $L_{2} \in 1L$ when $L = NL$. In this way, we can make a relation between every language $L_{1} \in 1LNL$ to another language $L_{2} \in 1L$. We can slightly modify this relation to create a new bijective relation just enumerating the languages $L_{i} \in 1LNL$ with a unique and natural number $i$ such that the relation is now with the corresponding language $L_{2, i}$ where
\[L_{2, i} = \{i \sqcup j: j \in L_{2}\}\]
and $\sqcup$ is the blank symbol \cite{Pap03}. We can make this because $1LNL$ is enumerable due to $P$ is enumerable and $1LNL \subseteq P$ \cite{Pap03}. However, we know for every natural number $i$ that $L_{2, i}$ belongs to $1L$, since $L_{2} \in 1L$ and we can check in a deterministic one-way computation the nonempty prefix substring $i$ without using any amount of space in the work tapes. Hence, we obtain that $|1LNL|_{c} < |1L|_{c}$ using this modified relation. However, this is not possible according to the Theorem \ref{partially}. Consequently, we prove the complexity class $L$ is not equal to $NL$.
\end{proof}
