...
has a cardinality greater than or equal to $\textit{1TAPE}[T'(n)]$. Nevertheless, the cardinality of the family of languages $\textit{1TAPE}[T'(n)]$ cannot be lesser than or equal to the cardinality of family of languages $\textit{1TAPE}[T'(0)]$. Therefore, we obtain that necessarily $\textit{1TAPE}[T'(n)]$ should be closed under deterministic two-way logarithmic space reduction by the reductio ad absurdum logic rule \cite{FE16}. In general, we can assume the same result for every sparse function $T(n)$.
\end{proof}
